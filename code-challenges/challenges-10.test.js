...
const count = (target, input) => {
  let repeatNumbers = Array.prototype.concat.apply([], input).filter(e => e === target)
  return repeatNumbers.length;
};
...
const totalSum = (input) => {
  let makeOneArray = Array.prototype.concat.apply([], input);
  return makeOneArray.reduce((a, b, c) => {
    return a = a + b
  }, 0)
};
...
let findMaleAndFemale = (data) => {
  ...
  return arrayNames.map(e => e.name).join(" and ");
};
